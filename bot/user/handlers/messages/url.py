...
async def url_handler(message: Message):
    if re.match(r'https?://(?:www\.)?(?:youtube\.com/.*[=/]|youtu\.be/)([\w-]+)', message.text) or re.match(r'https?://vm.tiktok.com/', message.text) or re.match(r'https?://(?:www\.)?tiktok\.com/.*', message.text):
        markup = InlineKeyboardBuilder()
        markup.add(types.InlineKeyboardButton(text='MP4', callback_data='format_mp4'))
        markup.add(types.InlineKeyboardButton(text='MP3', callback_data='format_mp3'))
...
@user_router.message(F.text)
async def download_handler(message: Message, format: str):
